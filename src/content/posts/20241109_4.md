...
tags: [blog]
category: Blog
description: issue経由でAstroのページに自動投稿をする

---

## 概要
Astroの記事をGitHub Issue経由で投稿するためのActionです。
titleをGitHubIssueに入れて文章を書き、labelに「publish」をつけるだけで投稿PRが作成されます
      ...
      - name: Create or Update Content File
        ...
        run: |
          ...
          content=$(echo "$ISSUE_BODY" | sed -e '0,/## By Issue/d' \
                    ...
                    -e "0,/^description: .*/s//description: $d/")
