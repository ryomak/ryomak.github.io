...
OAuthは、「別サービスへのアクセス許可」を受け取る流れである
...
OAuthをログインに使うなら、OpenID Connectで本人情報を扱う前提にする

| 用語 | 目的 | 実装で見るもの |
| --- | --- | --- |
| OAuth 2.0 | 権限委譲 | access token、scope、resource server |
| OpenID Connect | 認証情報の受け渡し | ID token、userinfo、nonce |
| ID token | クライアント向けの本人情報 | `iss`、`aud`、`sub`、`exp`、`nonce` |
| access token | API向けの許可 | scope、audience、期限、署名 |

- 「Googleログイン」はOIDCの文脈でも理解する
- ID tokenをAPI認可に使わず、access tokenと役割を分ける
- `sub` はユーザー名として扱わず、発行者内での主体IDとして扱う

---

## 401と403を分ける

認証認可のバグは、エラーコードを分けるだけでも調査しやすくなる

| 状態 | 返す候補 | 意味 |
| --- | --- | --- |
| ログインしていない | 401 | 誰か分からない |
| トークン期限切れ | 401 | 再認証または再発行が必要 |
| 権限が足りない | 403 | 誰かは分かるが許可できない |
| リソースを隠したい | 404 | 存在自体を見せない設計もある |

- 401を返す時は、クライアントが再ログインすべきか再発行すべきかを決められる情報を持つ
- 403を返す時は、権限不足なのか契約不足なのかをログに残す
- 監査ログでは、ユーザー、リソース、操作、判断理由を分けて記録する

---

## 保存場所の判断

トークン設計では、保存場所が攻撃時の被害範囲を決める

| 保存場所 | 強いところ | 弱いところ |
| --- | --- | --- |
| HttpOnly Cookie | JavaScriptから読まれにくい | CSRF対策が必要 |
| memory | ページを閉じると消える | リロードで失われる |
| localStorage | 実装が簡単 | XSS時に読み取られやすい |
| server session | 失効と監査がしやすい | ストアの可用性が必要 |

<div class="quote-panel">ブラウザに長寿命の秘密を置かない。置くなら、盗まれた時の検知と失効まで設計に入れる。</div>

---

## 本番前の確認

認証認可は、動くことより「壊れた時に止められること」を確認する

| 観点 | 確認すること |
| --- | --- |
| Redirect URI | 完全一致で検証し、ワイルドカードを避ける |
| Token lifetime | access tokenは短く、refresh tokenはrotationする |
| Scope | APIごとに最小権限になっているか |
| Key rotation | JWKSの `kid` 変更に追従できるか |
| Logout | セッション、refresh token、端末単位の失効ができるか |
| Audit | ログイン失敗、権限変更、token exchangeを追えるか |

---

## まとめ

用語を覚えるより、状態・保存場所・失効方法を分けると設計できる

- OAuthは権限委譲、OIDCはログイン文脈の本人情報を扱う
- JWTは形式であり、ログイン方式そのものではない
- Cookieは入れ物で、セッションはサーバー側状態の持ち方である
- ブラウザに置く秘密を減らすほど、XSS時の被害範囲は小さくなる
